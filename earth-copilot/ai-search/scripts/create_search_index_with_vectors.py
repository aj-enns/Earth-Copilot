...
def get_search_credential():
    """Get credential for Azure AI Search - supports both API key and Managed Identity"""
    if USE_MANAGED_IDENTITY or not SEARCH_KEY:
        print("Using Managed Identity for Azure AI Search authentication")
...
print(f"SEARCH_KEY: {'***' if SEARCH_KEY else 'None (using Managed Identity)'}")
...
if not SEARCH_KEY and not USE_MANAGED_IDENTITY:
    missing_vars.append("SEARCH_KEY (or set USE_MANAGED_IDENTITY=true)")
if not INDEX_NAME:
...
def delete_existing_index():
    ...
    try:
        index_client = SearchIndexClient(
            endpoint=SEARCH_ENDPOINT, credential=get_search_credential()
        )
        index_client.delete_index(INDEX_NAME)
        print(f"[OK] Successfully deleted existing index: {INDEX_NAME}")
...
def create_vector_index():
    ...
    try:
        ...
        print(f"[OK] Successfully created vector index: {INDEX_NAME}")
        return result
    except Exception as e:
...
def populate_vector_index():
    ...
    try:
        ...
        print(f"[FAIL] Error initializing embeddings: {e}")
        raise
    ...
    try:
        ...
        print(f"[OK] Fetched {len(collections.get('collections', []))} collections")
    except Exception as e:
        print(f"[FAIL] Error fetching collections: {e}")
        raise
    ...
    try:
        ...
        print("[OK] Search client initialized")
    except Exception as e:
        print(f"[FAIL] Error initializing search client: {e}")
        raise
    ...
    for i, collection in enumerate(collections.get("collections", []), 1):
        ...
        try:
            ...
            print(f"  [FAIL] Error generating embedding for {collection['id']}: {e}")
            continue
    ...
    if documents:
        ...
        print("[FAIL] No documents to upload")
...
if __name__ == "__main__":
    try:
        ...
        print("[OK] Vector index created and populated successfully!")
