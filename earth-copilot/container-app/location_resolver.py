...
class EnhancedLocationResolver:
    """
    [GLOBE] MULTI-STRATEGY LOCATION RESOLVER
    ...
    STORED_LOCATIONS = {
        ...
        'beirut': [35.47, 33.85, 35.55, 33.92],
        ...
        'kathmandu': [85.20, 27.60, 85.45, 27.80],
        'kathmandu valley': [85.15, 27.55, 85.55, 27.82],
        ...
        'paradise': [-121.72, 39.66, -121.52, 39.86],          # Paradise, CA (2018 Camp Fire)
        'southern california': [-120.50, 32.50, -116.00, 35.80], # SoCal region
        'socal': [-120.50, 32.50, -116.00, 35.80],              # Alias
        ...
        # === EXTREME WEATHER MODULE – Navigate Query Cities ===
        'dhaka': [90.32, 23.65, 90.50, 23.90],            # Capital of Bangladesh
        'dhaka bangladesh': [90.32, 23.65, 90.50, 23.90],  # Alias
        'maputo': [32.45, -26.10, 32.65, -25.85],          # Capital of Mozambique
        'maputo mozambique': [32.45, -26.10, 32.65, -25.85],  # Alias
        'niamey': [2.00, 13.42, 2.22, 13.58],            # Capital of Niger (Sahel)
        'niamey niger': [2.00, 13.42, 2.22, 13.58],       # Alias
        'san juan': [-66.15, 18.38, -65.98, 18.52],       # Capital of Puerto Rico
        'san juan puerto rico': [-66.15, 18.38, -65.98, 18.52],  # Alias
        'manaus': [-60.12, -3.20, -59.87, -2.98],         # Amazon basin city, Brazil
        'manaus brazil': [-60.12, -3.20, -59.87, -2.98],  # Alias
        'jalalabad': [70.35, 34.35, 70.55, 34.55],            # Eastern Afghanistan
        'jalalabad afghanistan': [70.35, 34.35, 70.55, 34.55], # Alias
        'el fasher': [25.25, 13.53, 25.45, 13.73],            # North Darfur, Sudan
        'el fasher sudan': [25.25, 13.53, 25.45, 13.73],      # Alias
        ...
        'mount rainier': [-121.88, 46.72, -121.60, 46.93],
        'mt rainier': [-121.88, 46.72, -121.60, 46.93],
        'mount st helens': [-122.28, 46.13, -122.08, 46.24],
        'mt st helens': [-122.28, 46.13, -122.08, 46.24],
        'denali': [-151.30, 62.92, -150.90, 63.15],
        ...
        # === MAJOR GEOGRAPHIC REGIONS (Climate / Extreme Weather queries) ===
        'southeast asia': [92.00, -11.00, 141.00, 28.50],
        'south east asia': [92.00, -11.00, 141.00, 28.50],  # Alias
        'se asia': [92.00, -11.00, 141.00, 28.50],  # Alias
        'caribbean': [-85.00, 10.00, -59.00, 27.50],
        'the caribbean': [-85.00, 10.00, -59.00, 27.50],  # Alias
        'caribbean region': [-85.00, 10.00, -59.00, 27.50],  # Alias
        'caribbean sea': [-85.00, 10.00, -59.00, 27.50],  # Alias
        'sahel': [-17.00, 10.00, 40.00, 20.00],
        'sahel region': [-17.00, 10.00, 40.00, 20.00],  # Alias
        'sahel region in africa': [-17.00, 10.00, 40.00, 20.00],  # Alias for Get Started query
        'the sahel': [-17.00, 10.00, 40.00, 20.00],  # Alias
        'east africa': [28.00, -11.75, 51.50, 12.00],
        'west africa': [-17.54, 4.27, 16.00, 16.70],
        'horn of africa': [32.00, -1.70, 51.50, 18.00],
        'central america': [-92.24, 7.20, -77.18, 18.50],
        'middle east': [25.00, 12.00, 63.50, 42.00],
        'central asia': [46.50, 35.00, 87.50, 55.50],
        'south asia': [60.00, 5.90, 97.50, 37.10],
        'east asia': [100.00, 18.00, 154.00, 53.60],
        'north africa': [-17.10, 15.00, 36.90, 37.60],
        'sub-saharan africa': [-17.60, -34.85, 51.50, 15.00],
        'southern africa': [11.70, -34.85, 40.85, -10.00],
    ...
    async def resolve_location_to_bbox(self, location_name: str, location_type: str = "region") -> Optional[List[float]]:
        ...
        # Try splitting on comma for "City, Country" patterns (e.g., "Bangkok, Thailand" -> "bangkok")
        for loc_variant in [location_lower, location_normalized]:
            if ',' in loc_variant:
        ...
        for query in [location_name] + processed_queries:
            ...
            if cached_bbox:
                self.logger.info(f"[LIST] Cache hit for {query}")
                return cached_bbox
    ...
    def _expand_bbox_for_large_features(self, bbox: List[float], location_name: str) -> List[float]:
        ...
        if is_large_feature:
            ...
            if current_extent < 0.5:  # Very small bbox (< ~35 miles)
                ...
                self.logger.info(f"[SEARCH] Expanding small bbox for large feature '{location_name}' by {expansion_factor}x")
            elif current_extent < 1.0:  # Small bbox (< ~69 miles)
                expansion_factor = 4.0
                self.logger.info(f"[SEARCH] Expanding bbox for large feature '{location_name}' by {expansion_factor}x")
            ...
            self.logger.info(f"[RULER] Expanded bbox from {bbox} to {expanded_bbox} (extent: {current_extent:.2f}° -> {max(new_width, new_height):.2f}°)")
...
Location: {location_name}"""
    ...
    def _preprocess_location_query(self, location_name: str, location_type: str) -> List[str]:
        """
        [BRAIN] Semantic Location Preprocessor
    ...
    async def _strategy_international_nominatim(self, location_name: str, location_type: str) -> Optional[List[float]]:
        """[GLOBE] International-focused Nominatim with smart query strategies"""
    ...
    def _validate_international_result(self, location_name: str, bbox: List[float]) -> bool:
        ...
        if name_lower in expected_regions:
            ...
            if not (min_lon <= center_lon <= max_lon and min_lat <= center_lat <= max_lat):
                self.logger.info(f"[NO] Rejected {location_name} result ({center_lon:.2f}, {center_lat:.2f}) - outside expected region")
                return False
        
        return True
    
    async def _strategy_geonames_alternative(self, location_name: str, location_type: str) -> Optional[List[float]]:
        """[DOCS] Alternative using free GeoNames-style approach via Nominatim with geographic focus"""
