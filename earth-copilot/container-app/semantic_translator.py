...
# Semantic Kernel 1.37.0+ compatible imports for GPT-4o support
try:
    ...
    SK_AVAILABLE = True
    logging.info(f"[OK] Semantic Kernel {sk.__version__} successfully imported with GPT-4o support")
except ImportError as e:
    SK_AVAILABLE = False
...
# GPT-4o can call during location extraction. When the fast keyword match
...
class GeocodingPlugin:
    ...
    This enables "True Agent with Tools" architecture where GPT-4o can:
                ...
                async with session.get(url, params=params, timeout=timeout) as response:
                    if response.status == 200:
                        ...
                        if features and len(features) > 0:
                            ...
                            name = locality or municipality or county or region or country or "Unknown Location"
...
class SemanticQueryTranslator:
    """Enhanced query translator using Semantic Kernel with GPT-4o for intelligent entity extraction and contextual Earth science analysis"""
    ...
    INTENT_TO_COLLECTIONS = {
        ...
        "crops": ["sentinel-2-l2a"],
        "agriculture": ["sentinel-2-l2a"],
        "npp": ["modis-17A3HGF-061"],
        "net primary production": ["modis-17A3HGF-061"],
        "primary production": ["modis-17A3HGF-061"],
        ...
        "ocean": ["sentinel-2-l2a"],
        "lake": ["sentinel-2-l2a"],
        "river": ["sentinel-2-l2a"],
        ...
        "fire": ["modis-14A2-061", "modis-14A1-061"],
        "wildfire": ["modis-14A2-061", "modis-14A1-061"],
        "thermal": ["modis-14A2-061", "modis-14A1-061"],
        "thermal anomalies": ["modis-14A2-061", "modis-14A1-061"],
        "daily fire": ["modis-14A1-061"],  # Explicit daily
        "fire daily": ["modis-14A1-061"],  # Explicit daily
        "thermal anomalies daily": ["modis-14A1-061"],
        "thermal anomalies 8-day": ["modis-14A2-061"],
        "fire 8-day": ["modis-14A2-061"],
        "modis fire": ["modis-14A2-061", "modis-14A1-061"],
        "modis fire daily": ["modis-14A1-061"],
        "modis fire 8-day": ["modis-14A2-061"],
        "modis thermal anomalies": ["modis-14A2-061", "modis-14A1-061"],
        "active fire": ["modis-14A1-061"],  # Daily is better for active/recent fires
        "recent fire": ["modis-14A1-061"],
        "temperature": ["landsat-c2-l2"],
        "heat": ["modis-14A2-061", "modis-14A1-061"],
        "burn": ["modis-64A1-061"],
        "burn severity": ["mtbs"],
        ...
        "classification": ["io-lulc-9-class"],
        "urban": ["io-lulc-9-class"],
        ...
        "s1": ["sentinel-1-rtc"],
        "rtc": ["sentinel-1-rtc"],
        "radiometrically terrain corrected": ["sentinel-1-rtc"],
        ...
        "nex gddp": ["nasa-nex-gddp-cmip6"],
        "nasa nex-gddp": ["nasa-nex-gddp-cmip6"],
        "nasa nex gddp": ["nasa-nex-gddp-cmip6"],
    ...
    def __init__(self, azure_openai_endpoint: str, azure_openai_api_key: str, model_name: str, azure_credential=None):
        ...
        # Store configuration for lazy initialization (GPT-4o optimized)
    ...
    def set_model(self, model_name: str) -> None:
        ...
        This allows the frontend to select between models (e.g., gpt-5, gpt-4o)
        without reinitializing the entire translator.
        
        Args:
            model_name: The deployment name of the model to use (e.g., 'gpt-5', 'gpt-4o')
        """
    ...
    async def classify_query_intent_unified(self, query: str, conversation_id: str = None) -> Dict[str, Any]:
        ...
        Single GPT-4o call that determines query intent and routing.
...
**[WARN] FOLLOW-UP DETECTION (HIGHEST PRIORITY):**
...
1. **vision**: Analyze CURRENTLY VISIBLE imagery (no new data loading)
   ...
   - Keywords: "in this image", "what's visible", "describe this", "what do you see", "can you see", 
               "on the map", "the map", "this area", "adjacent to", "near", "in this view"
   - **CRITICAL**: If user mentions a place name that's within/near the current view, this is vision NOT stac!
   - Examples: 
     ...
     * "What is the city on the map?" <- VISION (asks about current view)
     * "What are the bodies of water adjacent to Larissa?" <- VISION (if Larissa is on map)
     * "Tell me about the river I see" <- VISION
     * "What features are near the coast?" <- VISION
...
2. **stac**: Load NEW satellite imagery (visualization only, no analysis)
   ...
   - **CRITICAL**: Must have explicit "show/load/display" keyword to be stac!
   - Examples:
     * "Show me Seattle"
...
**[WARN] CRITICAL: Check query tense FIRST before anything else!**
...
0. **FOLLOW-UP CHECK (HIGHEST PRIORITY):**
   - If has_rendered_map=Yes AND query mentions "the map" / "this area" / "adjacent to" / visible features -> **vision**
   - If has_rendered_map=Yes AND query is about features/places in current view -> **vision**
   - Follow-up questions without "show/load/display" -> analyze current view = **vision**
...
2. **VISUALIZATION KEYWORDS:**
   - "in this image" / "what's visible" / "can you see" / "on the map" / "the map" -> **vision**
...
4. **IMPORTANT:**
   ...
   - **If user references current view context (has_rendered_map=Yes) without "show/load", it's vision NOT stac!**
...
Query: "What is the city on the map?" (has_rendered_map=Yes)
Response: {{"intent_type": "vision", "needs_satellite_data": false, "needs_vision_analysis": true, "needs_contextual_info": false, "confidence": 0.97, "reasoning": "References 'the map' with data displayed - analyze current view"}}

Query: "What are the bodies of water adjacent to Larissa?" (has_rendered_map=Yes, Larissa visible)
Response: {{"intent_type": "vision", "needs_satellite_data": false, "needs_vision_analysis": true, "needs_contextual_info": false, "confidence": 0.96, "reasoning": "Follow-up asking about features near a location in current view"}}

Query: "Tell me about the river I see"
Response: {{"intent_type": "vision", "needs_satellite_data": false, "needs_vision_analysis": true, "needs_contextual_info": false, "confidence": 0.97, "reasoning": "References visible feature - analyze current view"}}
...
"""
    ...
    async def _ensure_kernel_initialized(self):
        ...
        try:
            ...
            # Use stable API version that works well with GPT-4o/GPT-5
            ...
            # Primary service: Selected model (gpt-5 or gpt-4o) for complex reasoning tasks
            ...
            # PERFORMANCE OPTIMIZATION: Add GPT-4o service for structured tasks
            # GPT-4o is faster and cheaper for collection mapping, datetime parsing, etc.
            try:
                if ad_token_provider:
                    azure_chat_service_4o = AzureChatCompletion(
                        deployment_name="gpt-4o",  # GPT-4o deployment name
                        ad_token_provider=ad_token_provider,
                        base_url=base_url,
                        api_version="2024-08-01-preview",  # GPT-4o API version
                        service_id="chat-completion-4o"  # Separate service ID
                    )
                else:
                    azure_chat_service_4o = AzureChatCompletion(
                        deployment_name="gpt-4o",  # GPT-4o deployment name
                ...
                logger.info(f"   [OK] AzureChatCompletion service created (GPT-4o)")
                
                self.kernel.add_service(azure_chat_service_4o)
                logger.info(f"   [OK] GPT-4o service added to kernel for performance optimization")
            except Exception as e:
                logger.warning(f"   [WARN] Failed to add GPT-4o secondary service: {e}")
                # Not fatal - will continue with primary GPT-4o service
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            execution_settings = AzureChatPromptExecutionSettings(
                temperature=1.0,  # GPT-4o default temperature
            ...
            # GPT-4o may return response with usage data but empty content in some cases
    ...
    async def collection_mapping_agent(self, query: str) -> List[str]:
        ...
        # ========================================================================
        # Try intent-based keyword matching first (10ms vs 1200ms LLM call)
...
==============================================================================
COLLECTION SELECTION LOGIC (TWO-STEP PROCESS)
==============================================================================

STEP 1: CHECK FOR EXACT DATASET NAME MATCH
-------------------------------------------
First, check if the query mentions ANY collection name from the catalog above.
This includes:
- Exact collection IDs (e.g., "3dep-lidar-dtm", "cop-dem-glo-30", "sentinel-2-l2a")
- Dataset names (e.g., "Sentinel-2", "Landsat", "MODIS", "Copernicus DEM", "HLS", "NAIP")

If query mentions a dataset name -> Return ONLY that collection ID.

CRITICAL - LANDSAT LEVEL SELECTION:
- "Landsat Collection 2 Level 1" or "Landsat Level 1" or "L1" -> ["landsat-c2-l1"]
- "Landsat Collection 2 Level 2" or "Landsat Level 2" or "L2" -> ["landsat-c2-l2"]
- "Landsat" (no level specified) -> ["landsat-c2-l2"] (default to L2)

Examples:
- "Show me 3dep-lidar-dtm for Colorado" -> ["3dep-lidar-dtm"]
- "Get Copernicus DEM elevation" -> ["cop-dem-glo-30"]
- "Sentinel-2 imagery of France" -> ["sentinel-2-l2a"]
- "Landsat Collection 2 Level 1 of NYC" -> ["landsat-c2-l1"]  (Level 1 specified!)
- "Landsat Level 1 imagery" -> ["landsat-c2-l1"]  (Level 1 specified!)
- "Landsat data for California" -> ["landsat-c2-l2"]  (no level = default L2)
- "HLS images" -> ["hls2-l30", "hls2-s30"]
- "MODIS fire data" -> ["modis-14A1-061"]
- "NAIP aerial imagery" -> ["naip"]


STEP 2: USE DESCRIPTIONS AND CATEGORIES (ONLY IF NO DATASET NAME MATCH)
------------------------------------------------------------------------
If query does NOT mention any dataset name, analyze:
1. The DESCRIPTIONS of all collections in the catalog
2. The CATEGORIES they belong to
3. Select the MOST AVAILABLE dataset in the matching category

Match by category context:
- Fire/thermal queries -> Pick most available FIRE collection (e.g., modis-14A1-061)
- Elevation/terrain queries -> Pick most available ELEVATION collection (e.g., cop-dem-glo-30 for global, 3dep-seamless for US)
- Optical imagery queries -> Pick most available OPTICAL collection (e.g., sentinel-2-l2a, landsat-c2-l2)
- SAR/radar queries -> Pick most available RADAR collection (e.g., sentinel-1-grd)
- Vegetation queries -> Pick most available VEGETATION collection (e.g., modis-13Q1-061)

Examples:
- "Show wildfire activity in California" -> ["modis-14A1-061"] (FIRE category, most available)
- "Elevation data for Colorado" -> ["3dep-seamless"] (ELEVATION category, US location)
- "Satellite images of Boston" -> ["sentinel-2-l2a"] (OPTICAL category, highest resolution)
- "Radar flood detection" -> ["sentinel-1-grd"] (RADAR category)


==============================================================================
IMPORTANT CONSTRAINTS
==============================================================================
- Return EXACTLY 1 collection (the single best match)
- Exception: HLS queries can return ["hls2-l30", "hls2-s30"] (same product family)
- Use EXACT collection IDs from the catalog above
- Always prioritize STEP 1 (dataset name match) over STEP 2 (description match)
- Do NOT mix different product families (e.g., Sentinel-2 + Landsat = BAD)
...
Format: ["collection-id"]"""
            ...
            # PERFORMANCE OPTIMIZATION: Use GPT-4o for structured tasks
            # Collection mapping benefits from GPT-4o's speed and accuracy
            # GPT-4o is 40-50% faster and more cost-effective for this use case
            execution_settings = AzureChatPromptExecutionSettings(
                service_id="chat-completion-4o",  # Use GPT-4o service
                temperature=0.3,  # Lower temperature for structured output (collection selection)
                max_completion_tokens=300  # Reduced tokens (just need JSON array)
            )
            logger.info(f"[OK] AGENT 1: Execution settings created (model=gpt-4o, temp=0.3, max_tokens=300)")
    ...
    def _fast_location_keyword_match(self, query: str) -> Optional[str]:
        ...
        If a location is NOT found here, the location_extraction_agent will use GPT-4o
        ...
        # ========================================================================
        # The location_resolver.py has the comprehensive list with bounding boxes.
...
[WARN]  Only essential collections available in this fallback mode.
[WARN]  Many collections (including 3DEP LiDAR) are NOT available.
================================================================================

ESSENTIAL OPTICAL:
- sentinel-2-l2a: Sentinel-2 optical imagery (10-60m)
- landsat-c2-l2: Landsat optical imagery (30m)
...
ESSENTIAL FIRE:
- modis-14A1-061: MODIS fire detection
...
================================================================================"""
        ...
        try:
            ...
            if location_name:
                ...
                if bbox and self._validate_bbox(bbox):
                    ...
                    # ================================================================
                    # HLS/Sentinel-2 tiles are approximately 1° x 1° (110km x 110km)
                    # Calculate area in square degrees and estimate tiles needed
            ...
            # ========================================================================
            logger.info("=" * 80)
            logger.info("[TARGET] AGENT 2 COMPLETED - FINAL STAC QUERY BUILT")
            logger.info("=" * 80)
    ...
    async def location_extraction_agent(self, query: str) -> Dict[str, Any]:
        ...
        # ========================================================================
        # [FAST] FAST KEYWORD EXTRACTION - Check for known locations BEFORE calling GPT
        # ========================================================================
        ...
        # This ensures the actual query text is sent to GPT-4o, not the literal "{{$query}}" string
...
Return only the JSON object. No explanations or additional text."""
        ...
        try:
            ...
            # PERFORMANCE OPTIMIZATION: Use GPT-4o for location extraction with function calling
            # Location extraction is a structured entity extraction task (extract city/region name)
            # GPT-4o is optimized for structured extraction tasks
            # Enable function calling so GPT-4o can use azure_maps_geocode tool for unknown locations
            execution_settings = AzureChatPromptExecutionSettings(
                service_id="chat-completion-4o",  # Use GPT-4o service
                temperature=0.3,  # Lower temperature for structured entity extraction
            ...
            logger.info("[TOOL] Function calling enabled - GPT-4o can use azure_maps_geocode tool")

            # Execute using simplified invoke_prompt (no template needed with f-string)
            arguments = KernelArguments(settings=execution_settings)
            
            logger.info(f"[SEARCH] Sending location extraction prompt to GPT-4o...")
            logger.info(f"[TOOL] Using GPT-4o (temp=0.3, max_tokens=300) for structured entity extraction")
            ...
            # [SEARCH] DEBUG: Log raw GPT-4o response
    ...
    async def tile_selector_agent(
        ...
        try:
            ...
            for coll_id in collection_ids:
                ...
                if coll_metadata:
                    res_str = coll_metadata.get("spatial_resolution", "unknown")
                else:
                    res_str = "unknown"
...
[TARGET] TOP PRIORITIES (MANDATORY):
...
SELECTION RULES:
[OK] MUST: Use only highest resolution tiles ({best_resolution}m)
[OK] MUST: Ensure 100% spatial coverage of area of interest
[OK] MUST: Select tiles from single acquisition datetime
[OK] SHOULD: Minimize number of tiles (fewer is better if coverage complete)
[OK] SHOULD: Respect cloud cover preferences from query
[OK] SHOULD: Choose most recent acquisition time available
[FAIL] NEVER: Mix different resolutions
[FAIL] NEVER: Leave gaps in spatial coverage
[FAIL] NEVER: Mix tiles from different acquisition times
...
- Prioritize quality over quantity"""
                ...
                # Use GPT-4o for tile selection (faster, more reliable than GPT-5 for structured selection)
                execution_settings = AzureChatPromptExecutionSettings(
                ...
                # Get chat completion service - prefer GPT-4o for tile selection
                # GPT-4o is faster and more reliable for structured selection tasks
                chat_completion = self.kernel.get_service(type=ChatCompletionClientBase)
                
                # Override to use GPT-4o if available (better for tile selection than GPT-5)
                # If GPT-4o deployment exists, use it; otherwise fall back to default model
                try:
                    gpt4o_service = None
                    for service_id, service in self.kernel._services.items():
                        if hasattr(service, 'ai_model_id'):
                            model_id = service.ai_model_id
                            # Check for GPT-4o deployment
                            if 'gpt-4o' in model_id.lower() or 'gpt4o' in model_id.lower():
                                gpt4o_service = service
                                logger.info(f"[OK] Using GPT-4o for tile selection: {model_id}")
                                break
                    
                    if gpt4o_service:
                        chat_completion = gpt4o_service
                    else:
                        logger.info(f"ℹ️ GPT-4o not found, using default model for tile selection")
                except Exception as e:
                    logger.warning(f"[WARN] Could not check for GPT-4o service: {e}, using default")
        ...
        try:
            ...
            for feature in stac_features:
                ...
                if coll in collection_ids:
                    coll_metadata = get_collection_metadata(coll)
                    if coll_metadata:
                        res_str = coll_metadata.get("spatial_resolution", "1000m")
                    else:
                        res_str = "1000m"
            ...
            for tile in selected_time_tiles:
                ...
                if not selected:
                    ...
                    # Check if tile overlaps with requested bbox and adds new coverage
                    ...
                    if west <= tile_center_lon <= east and south <= tile_center_lat <= north:
                        ...
                        for sel_tile in selected:
                            sel_bbox = sel_tile.get("bbox", [])
                            if len(sel_bbox) == 4:
                                sw, ss, se, sn = sel_bbox
                                # If tiles overlap significantly, don't add
                                overlap_w = max(tw, sw)
                                overlap_e = min(te, se)
                                overlap_s = max(ts, ss)
                                overlap_n = min(tn, sn)
    ...
    async def _add_cloud_filtering_to_query(self, stac_query: Dict[str, Any], query: str, entities: Dict[str, Any], collections: List[str]) -> None:
        ...
        if cloud_intent is None:
            ...
            cloud_filterable = [c for c in collections if supports_cloud_filtering(c)]
            if cloud_filterable:
    ...
    async def datetime_translation_agent(
        ...
        try:
            ...
            # PERFORMANCE OPTIMIZATION: Use GPT-4o for datetime parsing
            # Datetime translation is a structured task that doesn't need GPT-5's reasoning
            # GPT-4o is 40-50% faster and handles temporal parsing very well
            execution_settings = AzureChatPromptExecutionSettings(
                service_id="chat-completion-4o",  # Use GPT-4o service
                temperature=0.3,  # Lower temperature for structured datetime output
                max_completion_tokens=300 if mode == "comparison" else 150  # Reduced tokens for JSON output
            )
            logger.info(f"[TOOL] AGENT 2.2: Using GPT-4o (temp=0.3, max_tokens={300 if mode == 'comparison' else 150})")
...
[WARN][WARN][WARN] CRITICAL: ALWAYS use the EXACT year mentioned in the query! [WARN][WARN][WARN]
DO NOT substitute with {current_year}! If the query says "2024", use 2024!
If the query says "2023", use 2023! If the query says "July 2024", use 2024-07!

WRONG EXAMPLES (DO NOT DO THIS):
[FAIL] Query: "fire data for California in July 2024" -> Response: "2025-07-01/2025-07-31" (WRONG! Changed 2024 to 2025!)
[FAIL] Query: "from 2024" -> Response: "2025-01-01/2025-12-31" (WRONG! Changed 2024 to 2025!)
...
Query: "Show me MODIS fire data for California in July 2024"
Response: {{"datetime_range": "2024-07-01/2024-07-31", "explanation": "Month and year: July 2024"}}
...
- **Low/Clear**: "clear", "cloudless", "no clouds", "cloud-free", "clear sky", "minimal clouds", "low cloud", "low cloud cover", "with low cloud"
...
- **High/Cloudy**: "cloudy", "overcast", "lots of clouds", "heavy clouds", "very cloudy", "high cloud cover"
...
Query: "Show recent Sentinel-2 imagery of Moscow with low cloud cover"
Response: {{"cloud_intent": "low", "threshold": 25, "reasoning": "User explicitly requested 'low cloud cover'"}}
...
Return ONLY a JSON object with "cloud_intent", "threshold", and "reasoning". No additional text."""
        ...
        try:
            ...
            # PERFORMANCE OPTIMIZATION: Use GPT-4o for cloud intent detection
            # Cloud filtering is a simple yes/no + threshold task, perfect for GPT-4o
            execution_settings = AzureChatPromptExecutionSettings(
                service_id="chat-completion-4o",  # Use GPT-4o service
                temperature=0.3,  # Lower temperature for structured output
                max_completion_tokens=100  # Reduced tokens for simple JSON response
            )
            logger.info(f"[TOOL] AGENT 2.3: Using GPT-4o (temp=0.3, max_tokens=100)")
...
Location to analyze: {location_name}"""
    ...
    async def translate_query(self, natural_query: str, pin_location: Optional[Dict[str, float]] = None, session_bbox: Optional[List[float]] = None, skip_intent_classification: bool = False, pre_classified_intent: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"[NOTE] Query: '{natural_query}'")
        if session_bbox:
        ...
        try:
            ...
            if data_source == "veda":
                ...
                logger.info("[GLOBE] Using VEDA STAC source")
            else:
...
Keep your response focused, informative, and directly relevant to the user's question."""
    ...
    def _determine_tile_limit(self, bbox: List[float], query: str) -> int:
        ...
        if area_km2 < 100:  # City-scale (~10km x 10km)
            ...
            base_limit = 60  # Increased from 40
        elif area_km2 < 500000:  # Country-scale (e.g., Greece ~130k km²)
    ...
    def _prepare_stac_data_summary(self, features: List[Dict], collection_summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        platform_fallback_map = {
            ...
            "hls2-l30": "NASA HLS (Landsat)",
            "hls2-s30": "NASA HLS (Sentinel-2)",
            "modis-14A1-061": "NASA MODIS",
            "modis-14A2-061": "NASA MODIS",
            "modis-64A1-061": "NASA MODIS",
    ...
    def _get_collection_details(self, collections: List[str]) -> Dict[str, str]:
        ...
        collection_descriptions = {
            ...
            "hls2-l30": "Harmonized Landsat Sentinel-2 L30 (30m resolution)",
            "hls2-s30": "Harmonized Landsat Sentinel-2 S30 (30m resolution)",
            "modis-14A1-061": "MODIS thermal anomalies and fire detection data",
            "modis-14A2-061": "MODIS thermal anomalies 8-day composite",
            "modis-64A1-061": "MODIS burned area product",
...
def process_query_with_openai_sync(query: str) -> Dict[str, Any]:
    ...
    """
    try:
        logger.info(f"[SYNC] Running synchronous wrapper for: '{query}'")
        result = asyncio.run(process_query_with_openai(query))
        return result
    except Exception as e:
        logger.error(f"[FAIL] Error in synchronous wrapper: {e}")
        return {
            "error": str(e),
            "query": query,
