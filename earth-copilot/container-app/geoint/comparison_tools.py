...
def _resolve_location_sync(location: str) -> Optional[List[float]]:
    ...
    Runs the async resolver in a dedicated thread with its own event loop
    so it never conflicts with the Agent SDK event loop.
    """
    try:
...
def _fetch_tile_preview_sync(tile_json_url: str) -> Optional[str]:
    """Fetch a representative tile image from a TileJSON URL and return as base64.

    Uses sync requests instead of aiohttp.
    """
    import base64
    try:
...
def analyze_comparison_imagery(location: str, before_tile_url: str, after_tile_url: str, analysis_type: str = "general") -> str:
    ...
    try:
        ...
        before_image = _fetch_tile_preview_sync(before_tile_url)
        after_image = _fetch_tile_preview_sync(after_tile_url)
