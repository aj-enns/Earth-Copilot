...
class ChatVisionAnalyzer:
    ...
    def should_use_vision(self, query: str, conversation_history: Optional[List[Dict]] = None) -> bool:
        ...
        vision_keywords = [
            ...
            "what's on the map",
            "what is on the map",
            "on the map",  # Any reference to "the map" implies current view
            "the map",  # Broader catch for map references
            "in this view",
            "in this area",
            "this region",
            "adjacent to",  # Follow-up questions about nearby features
            "near the",  # Follow-up: "what's near the coast"
            "what is shown here",
            "identify features",
            "what's visible",
            "what city is this",  # Follow-up asking about current view
            "what river is this",
            "what lake is this",
    ...
    async def analyze_visible_imagery(
        ...
        """
        try:
            logger.info(f" Analyzing visible imagery for query: '{query}'")
    ...
    async def _fetch_visible_imagery(
        ...
        try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(imagery_url, timeout=aiohttp.ClientTimeout(total=30)) as response:
                        if response.status == 200:
                            ...
                            logger.warning(f" Failed to fetch from provided URL: {response.status}")
            ...
            logger.warning(" No imagery available to analyze")
