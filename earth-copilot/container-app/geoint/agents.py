...
# LAZY IMPORTS: Agent singletons are created on first use via get_*_agent() functions.
# Each agent module provides its own singleton accessor.
...
async def terrain_analysis_agent(
    ...
    """
     Terrain Analysis Agent - Full SK Agent with DEM tools + GPT-4o Vision
    ...
    - Flat area detection (landing zones, camps)
    - Visual analysis (GPT-4o Vision on satellite imagery)
    ...
    Returns:
        ...
        - response: str (agent response with tool results)
    ...
    try:
        ...
        logger.info(f" Terrain analysis agent called for ({latitude}, {longitude})")
        
        # Get singleton agent (preserves sessions across requests)
        agent = _get_terrain_agent()
        
        # Generate session ID if not provided (for single-shot requests)
        if not session_id:
            session_id = f"terrain_{uuid.uuid4().hex[:8]}"
        
        # Convert radius from miles to km
        ...
        # Format response for consistency with other agents
        response = {
            "agent": "terrain_analysis_agent",
            "response": result.get("response", ""),
            "tool_calls": result.get("tool_calls", []),
            "session_id": session_id,
            "analysis": result.get("response", ""),  # Alias for compatibility
            "features": [],  # Will be populated by tool results
            "confidence": 0.85 if result.get("tool_calls") else 0.7,
            "imagery_metadata": {
                "source": "Copernicus DEM + GPT-4o Vision",
                "resolution": "30m (DEM) + 10m (Sentinel-2)"
            }
        }
        
        # Extract features from tool calls
        if result.get("tool_calls"):
            response["features"] = [f"DEM analysis via {tool}" for tool in result["tool_calls"]]
        
        logger.info(f" Terrain analysis completed with {len(result.get('tool_calls', []))} tool calls")
        return response
...
async def mobility_analysis_agent(
    ...
    """
     Mobility Analysis Agent - Two-point traverse trafficability assessment
    
    Analyzes terrain traversability between Point A and Point B using:
    ...
    Args:
        latitude: Point A latitude (start)
        longitude: Point A longitude (start)
        ...
        latitude_b: Point B latitude (destination, optional)
        longitude_b: Point B longitude (destination, optional)
    ...
    try:
        if latitude_b is not None and longitude_b is not None:
            logger.info(f" Mobility analysis agent: A({latitude}, {longitude}) -> B({latitude_b}, {longitude_b})")
        else:
            logger.info(f" Mobility analysis agent called for ({latitude}, {longitude})")
        
        # Build query with A->B context if both points provided
        effective_query = user_query
        if latitude_b is not None and longitude_b is not None and not user_query:
            effective_query = (
                f"Analyze traversability from Point A ({latitude:.4f}, {longitude:.4f}) "
                f"to Point B ({latitude_b:.4f}, {longitude_b:.4f}). "
                f"Assess GO/SLOW-GO/NO-GO terrain classification along the route between these two points."
            )
        
        agent = _get_mobility_agent()
        result = await agent.analyze_mobility(
            latitude=latitude,
            longitude=longitude,
            user_context=effective_query,
            include_vision_analysis=include_vision,
            screenshot_base64=screenshot_base64,
...
async def building_damage_agent(
    ...
    """
     Building Damage Assessment Agent - Structural damage analysis
    ...
    """
    try:
        logger.info(f" Building damage agent called for ({latitude}, {longitude})")
        
        agent = _get_building_damage_agent()
        result = await agent.analyze_building_damage(
            latitude=latitude,
            longitude=longitude,
...
async def extreme_weather_analysis_agent(
    latitude: float,
    longitude: float,
    screenshot_base64: Optional[str] = None,
    user_query: Optional[str] = None,
    ...
    """
     Extreme Weather & Climate Projection Agent — NASA NEX-GDDP-CMIP6
    
    Analyzes future climate projections using downscaled CMIP6 data:
    - Temperature projections (max, min, mean in °C)
    - Precipitation projections (mm/day)
    - Wind speed projections (m/s)
    - Humidity projections (relative %, specific g/kg)
    - Radiation projections (W/m²)
    - Scenario comparison (SSP2-4.5 vs SSP5-8.5)
    
    Data is NetCDF (not COG) — point-sampling only, no map tiles rendered.
    
    Args:
        latitude: Center latitude for analysis
        longitude: Center longitude for analysis
        screenshot_base64: Optional base64-encoded screenshot (not used for climate data)
        user_query: User context or specific question
        session_id: Optional session ID for multi-turn conversation
        
    Returns:
        Dict with:
        - agent: "extreme_weather_analysis_agent"
        - response: str (agent response with climate data)
        - tool_calls: List[str] (tools invoked)
        - session_id: str (for follow-up questions)
    """
    try:
        ...
        logger.info(f" Extreme weather agent called for ({latitude}, {longitude})")
        
        agent = _get_extreme_weather_agent()
        
        if not session_id:
            session_id = f"climate_{uuid.uuid4().hex[:8]}"
        
        query = user_query or "Provide a comprehensive climate projection overview for this location."
        ...
        if result.get("tool_calls"):
            response["features"] = [f"Climate analysis via {tc.get('tool', tc) if isinstance(tc, dict) else tc}" for tc in result["tool_calls"]]
        
        logger.info(f" Extreme weather analysis completed with {len(result.get('tool_calls', []))} tool calls")
        return response
        
    except Exception as e:
        logger.error(f" Extreme weather agent failed: {e}")
        raise
...
async def _download_and_visualize_raster(
    ...
    try:
        ...
        logger.info(f" Downloading raster for {collection_id} at {date}")
        ...
        if not items:
            logger.warning(f" No raster data found for {collection_id} at {date}")
            return None
...
async def comparison_analysis_agent(
    ...
    try:
        ...
        logger.info(f" Collection: {collection_id or 'N/A'}")
        ...
        if download_rasters and collection_id:
            ...
            if before_raster_base64:
                ...
                logger.warning(" BEFORE raster download failed or returned None")
            ...
            if after_raster_base64:
                ...
                logger.warning(" AFTER raster download failed or returned None")
            ...
            if before_raster_base64 and after_raster_base64:
                ...
                logger.warning(" Raster download incomplete, proceeding with screenshots only")
        else:
            logger.info("[SKIP] Raster download skipped (download_rasters=False or no collection_id)")
        ...
        logger.info(f" Raster type detected: {raster_type}")
        logger.info("")
...
"""
        ...
        logger.info(" Building image list for GPT-5 Vision:")
        image_count = 0
        ...
        logger.info(f"[TIME]  Time span: {_calculate_time_span(before_date, after_date)}")
    ...
    **kwargs
    ...
    try:
        ...
        module_map = {
            ...
            # "animation": animation_generation_agent  # DISABLED - Coming Soon
        }
