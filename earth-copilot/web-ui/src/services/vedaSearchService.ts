...
class VEDASearchService {
  ...
  constructor() {
    ...
    this.config = {
      ...
      openaiDeployment: import.meta.env.VITE_AZURE_OPENAI_DEPLOYMENT || 'gpt-4o'
    };
  }
  ...
  private async searchCollections(query: string, collection_id?: string): Promise<VEDACollection[]> {
    ...
    try {
      ...
      if (!queryVector) {
        console.log(' No query vector available, using text search only');
    ...
    try {
      ...
      try {
        ...
        if (collections.length === 0) {
          console.log(' No indexed collections found, using hybrid approach...');
