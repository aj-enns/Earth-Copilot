...
class ApiService {
  ...
  async sendChatMessage(
    ...
    try {
      ...
      const requestData: any = {
        query: message,
        model: selectedModel || 'gpt-4o',  // Default to GPT-4o
        preferences: {
  ...
  async triggerGeointAnalysis(
    ...
    try {
      ...
      // Add screenshot for all GEOINT analysis modules
      if (screenshot) {
        requestData.screenshot = screenshot;
      ...
      const response = await this.api.post(endpoint, requestData, {
        ...
        timeout: 240000, // 4 minutes for GEOINT (climate NetCDF downloads are slow)
      });
