...
const MapView: React.FC<MapViewProps> = ({ 
        ...
        if (selectedModule === 'extreme_weather' && navigateToData.latitude && navigateToData.longitude) {
          ...
          if (onGeointAnalysis) {
            ...
            onGeointAnalysis({
              ...
              message: `**Location Updated: ${locationName}**\n\nWhat would you like to know about the climate here? You can ask about temperature projections, precipitation trends, extreme heat days, and more.`,
          ...
          if (hasBothTiles) {
            ...
            (async () => {
              try {
                ...
                const visionResp = await fetch(`${API_BASE_URL}/api/geoint/comparison`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    latitude: center_lat,
  ...
  const handleTerrainAnalysisClick = async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (onGeointAnalysis) {
        onGeointAnalysis({
          type: 'terrain_ready',
          message: '**Terrain Pin Placed**\n\nWhat would you like to know about this location? You can ask about elevation, slope, vegetation, landforms, soil type, route feasibility, and more.',
          coordinates: { lat, lng }
        });
      }
            ...
            if (successCount > 0) {
              ...
              if (collection.toLowerCase().includes('modis') && satelliteData.bbox) {
                ...
                if (safeMSouth < safeMNorth) {
                  ...
                  console.log(`MapView: [MODIS FIX] Fitted bounds with minZoom=7 for fire detection`);
                }
  ...
  const handleModuleSelect = (module: string) => {
    ...
    if (module === 'vision') {
      ...
      message = '**Mobility Assessment — Two-Point Traverse**\n\n**Step 1:** Click on the map to drop **Pin A** (start point).\n**Step 2:** Click again to drop **Pin B** (destination).\n\nTraversability will be analyzed between the two points.';
      ...
      message = 'Please drop a pin on a building for damage analysis.';
  ...
  const handleMapClickForPin = async (lat: number, lng: number) => {
    ...
    if (selectedModule === 'comparison') {
      ...
      if (onGeointAnalysis) {
        onGeointAnalysis({
          type: 'assistant',
          message: `**Pin placed** at (${lat.toFixed(4)}°, ${lng.toFixed(4)}°)\n\nNow describe what you'd like to compare in the chat.`
        });
      }
    ...
    if (selectedModule === 'mobility') {
      ...
      if (onGeointAnalysis) {
        onGeointAnalysis({
          type: 'mobility_ready',
          message: `**Pins Placed**\n\n**A:** (${pinA.lat.toFixed(4)}°, ${pinA.lng.toFixed(4)}°)\n**B:** (${lat.toFixed(4)}°, ${lng.toFixed(4)}°)\n\nWhat would you like to know about traversability between these points? Type your question in the chat below.`,
    ...
    try {
      ...
      if (onGeointAnalysis) {
        ...
        if (selectedModule === 'building_damage') {
          ...
          console.log('[SNAP] MapView: Capturing screenshot for building damage analysis...');
          ...
          // Store screenshot in state so follow-up chat messages can access it via mapContext
          setVisionScreenshot(screenshot);
          ...
          onGeointAnalysis({
            type: 'extreme_weather_ready',
            message: '**Location Set**\n\nWhat would you like to know about the climate at this location? You can ask about temperature projections, precipitation trends, extreme heat days, and more.',
            coordinates: { lat, lng }
          });
