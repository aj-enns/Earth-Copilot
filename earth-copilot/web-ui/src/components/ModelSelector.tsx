...
const ModelSelector: React.FC<ModelSelectorProps> = ({ onModelChange, selectedModel, apiBaseUrl = '' }) => {
  ...
  const [currentModel, setCurrentModel] = useState<string>(
    selectedModel || DEFAULT_MODELS.find(m => m.isDefault)?.id || 'gpt-4o'
  );
  ...
  useEffect(() => {
    const fetchAvailableModels = async () => {
      try {
        ...
        // If the OpenAI service is configured/connected, mark gpt-4o as available.
        ...
        if (isOpenAiOk) {
          // Mark gpt-4o as available since OpenAI is configured
          setModels(prevModels => 
            prevModels.map(model => ({
              ...model,
              isAvailable: model.id === 'gpt-4o' ? true : model.isAvailable ?? false
            }))
          );
        }
      } catch (err) {
