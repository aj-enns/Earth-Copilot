...
const MainApp: React.FC<MainAppProps> = ({ appState, onDatasetSelect, onReturnToLanding, onRestartSession, geointMode, onGeointToggle, selectedModel }) => {
  ...
  useEffect(() => {
    ...
    const handleComparisonQueryEvent = (event: CustomEvent<{ query: string }>) => {
      console.log('MainApp: Received comparison query event:', event.detail.query);
      // Auto-select comparison module
      handleModuleSelected('comparison');
      // Set the comparison query to be processed by MapView
      setComparisonUserQuery(event.detail.query);
